...
{
	ffstr str = FFSTR_INITN(s, len), ip, sn;
	ffstr_rsplitby(&str, '/', &ip, &sn);
	ffuint r = ffip6_parse(ip6, ip.ptr, ip.len);
	if (r != 0)
		return -1;
...
static inline ffuint ffip6_tostr(const void *addr, char *dst, ffsize cap)
{
	...
	for (int i = 0;  i < 16; ) {
		...
		if (i != 0)
			*p++ = ':';
		p += ffs_fromint(ffint_be_cpu16_ptr(&a[i]), p, end - p, FFINT_HEXLOW); // convert 16-bit number to string
		i += 2;
	}
