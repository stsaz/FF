...
/** Create thread pool. */
FF_EXTN ffthpool* ffthpool_create(ffthpoolconf *conf);

/** Free thread pool object. */
FF_EXTN int ffthpool_free(ffthpool *p);
...
/** Create new task. */
FF_EXTN ffthpool_task* ffthpool_task_new(uint addsize);

/** Free task object. */
FF_EXTN void ffthpool_task_free(ffthpool_task *t);

/** Add task to the queue.  Thread-safe.
Create additional threads when necessary. */
FF_EXTN int ffthpool_add(ffthpool *p, ffthpool_task *task);
